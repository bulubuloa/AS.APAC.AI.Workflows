...
Fetch the Jira ticket referenced by $ARGUMENTS and create a task file in `issues/`. Fetch only — no code reading, no analysis, no fixes.

## Step 1 — Parse input

`$ARGUMENTS` is a key (`ABE-5457`) or a browse URL (`https://internationalsos.atlassian.net/browse/ABE-5457`). Extract the key.
