...
- Repo aliases: Bitbucket `apac-benefit-vendor-backend` = local `Omnicasa.Mobile.ABVB`; `apac-benefits-frontend` = local `Omnicasa.Mobile.ABF` (both under /Users/hoangq/Projects/OmnicasaAS). Integration branch is `origin/develop` in both. Bitbucket push/fetch fails locally (auth) — user pushes themselves.
- ABE-4325 (5 low-priority dynamic CMS modules) was re-implemented from scratch on 2026-06-05 because the old branch `jira/ABE-4325-dynamic-cms-module` (ABVB, commit d6e2b54) was declared incorrect/incomplete by the user (missed VendorConstants, DB mapping SQL, and all frontend work). New work lives in worktrees: `Omnicasa.Mobile.ABVB-ABE-4325` (branch `jira/ABE-4325-dynamic-cms-module-v2`) and `Omnicasa.Mobile.ABF-ABE-4325` (branch `jira/ABE-4325-dynamic-cms-module`).
