...
# Catches Atlassian API tokens, AWS access keys, JWTs, and password values (letters+digits, 8+ chars) after -p / pwd= / password=.
# Branch names like jira/ABE-5320-porsche-dealer are not matched (no digit inside the token, or followed by '-').
my $bad = 0;
while (<STDIN>) {
    next unless /^\+/ && !/^\+\+\+/;
    if (/ATATT3|AKIA[0-9A-Z]{12}|eyJ[A-Za-z0-9_-]{30,}/
