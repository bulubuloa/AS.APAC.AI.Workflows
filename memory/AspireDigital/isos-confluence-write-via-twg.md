...
Part 0 (setup kit) page `6838812674`, step-by-step Setup guide `6837862577` (child of Part 0). The kit itself: `AspireDigital/ai-workspace` (git; memory dirs for AspireDigital/ABCB/ABMB are symlinks into `ai-workspace/memory/`, task files in `ai-workspace/issues/`). At the end of a ticket: commit the memory + issues changes there.

CMS-only provider audit (ABE-5439) is on Confluence: parent `6838452246` 'Roadside use vendor from CMS' + 34 child pages (ids in `ai-workspace/confluence/cms-provider-audit/state.json`); the Lambda reader `cms-only-provider-audit-docs` is superseded and can be deleted.
