...
Produce evidence for every acceptance criterion in $1 and write it under `## Verification`. "It builds" is not evidence.
...
For each `- [ ]` acceptance criterion in $1 decide the check: browser path (page, action, expected), DB query (before/after), API call, log search (`aws logs filter-log-events`), unit test.
...
```
Tick the acceptance criteria that passed (`- [x]`). Fix real defects you found (back to code, new commit, re-verify) and add them to the table. Then suggest `/task-deliver $1`.
