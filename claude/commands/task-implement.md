...
Implement the task in $1 following its `## Analysis → Plan`. Stop at committed branches; pushing, PRs and shared-environment writes are the developer's (see `/task-deliver`).

## Rules
- The task file must contain a `## Analysis` with a Plan. If it does not, stop and say to run `/task-analyse` first.
...
```
Then say what is ready and suggest `/task-verify $1`.
