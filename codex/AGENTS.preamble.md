...
- **At the start of a session**, read `ai-workspace/memory/AspireDigital/MEMORY.md` (the index) and, when working inside a repo, the repo's index (`ai-workspace/memory/ABCB/MEMORY.md`, `…/ABMB/MEMORY.md`). Open the notes the index points at when they are relevant to the ticket — they hold environment ids, drifted stored procedures, pipeline quirks and incidents that are not derivable from the code.
